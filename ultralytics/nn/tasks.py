    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                # if f is a list/tuple (e.g., [-1, 6])
                if isinstance(m.f, (list, tuple)):
                    # get multiple inputs
                    x = [y[j] if j >= 0 else x for j in m.f]
                else:
                    x = y[m.f] if isinstance(
                        m.f, int) else x  # from earlier layers
            if profile:
                self._profile_one_layer(m, x, dt)
            if isinstance(m, CrossAttention):
                if not isinstance(x, (list, tuple)) or len(x) != 2:
                    raise ValueError(
                        f"CrossAttention expects two inputs (q, kv), but got {x}")
                x = m(x[0], x[1])  # call with q and kv
            else:
                x = m(x)  # run
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in base_modules:
            ...
            c2 = ch[f[0]]  # Output channels = in_channels_q
