...
class SAVPE(nn.Module):
    ...
    def __init__(self, ch: List[int], c3: int, embed: int):
        ...
        self.cv1 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c3, 3), Conv(c3, c3, 3), nn.Upsample(
                    scale_factor=i * 2) if i in {1, 2} else nn.Identity()
            )
            for i, x in enumerate(ch)
        )

        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 1), nn.Upsample(
                scale_factor=i * 2) if i in {1, 2} else nn.Identity())
            for i, x in enumerate(ch)
        )
    ...
    def forward(self, x: List[torch.Tensor], vp: torch.Tensor) -> torch.Tensor:
        ...
        y = y.reshape(B, 1, self.c, H, W).expand(-1, Q, -
                                                 1, -1, -1).reshape(B * Q, self.c, H, W)
        ...
        aggregated = score.transpose(-2, -3) @ x.reshape(B,
                                                         self.c, C // self.c, -1).transpose(-1, -2)
...
class C3k2Ghost(nn.Module):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        if c3k:
            self.m = nn.Sequential(
                *(C3k(c_, c_, shortcut, g, k=3, e=1.0) for _ in range(n)))
        else:
            self.m = nn.Sequential(
                *(GhostBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
